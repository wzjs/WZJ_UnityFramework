...
public static class InfoPlistProcessor
{
    ...
    private static void SetSelfDefineData(PlistDocument plist)
    {
        PlistElementDict rootDict = plist.root;

        var dictElement = plist.root.CreateDict("NSAppTransportSecurity");
        dictElement.SetBoolean(XcodeProjectSetting.NSALLOWS_ARBITRARYLOADS, true);

        plist.root.SetString("NSPhotoLibraryUsageDescription", "游戏需要访问您的相册权限");
        rootDict.SetBoolean(XcodeProjectSetting.UI_STATUSBARHIDDEN, true);
