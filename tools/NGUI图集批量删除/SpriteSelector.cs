...
public class SpriteSelector : ScriptableWizard
{
					...
					for (; offset < sprites.size; ++offset)
					{
						...
						if (GUI.Button(rect, ""))
						{
							if (Event.current.button == 0)
							{
                                ...
                                if (NGUISettings.selectedSprites.Contains(sprite.name))
                                    NGUISettings.selectedSprites.Remove(sprite.name);
                                else
                                    NGUISettings.selectedSprites.Add(sprite.name);
							...
							else
							{
								...
								NGUIContextMenu.AddItem("Delete", false, DeleteSprites, NGUISettings.selectedSprites);
						...
						if (Event.current.type == EventType.Repaint)
						{
                            ...
                            if (NGUISettings.selectedSprites.Contains(sprite.name))
                            {
                                NGUIEditorTools.DrawOutline(rect, new Color(0.8f, 1f, 0f, 1f));
                            }
                        }
    ...
    void DeleteSprites(object obj)
    {
        if (this == null) return;
        var sd = obj as List<string>;
        ...
        NGUISettings.selectedSprites.Clear();
        UIAtlasMaker.UpdateAtlas(NGUISettings.atlas, sprites);
        NGUIEditorTools.RepaintSprites();
    }
